...
def inferNetwork(iid, arch, endianness, init, try_wrap):
    global SCRIPTDIR
    global SCRATCHDIR
    ...
    lastWrappedFile = ""
    lastWrappedPath = targetDir + '/wrapped_file'
    if os.path.exists(lastWrappedPath):
        with open(targetDir + '/wrapped_file', 'r') as wrapFile:
            lastWrappedFile = wrapFile.read().strip()
            if os.path.exists(lastWrappedFile):
                os.remove(lastWrappedFile)
        wrapFile.close()
    ...
    if not init.endswith('preInit.sh'): # rcS, preinit
        if fileType.find('ELF') == -1 and fileType.find("symbolic link") == -1: # maybe script
            targetFile = os.path.join(targetDir + '/image/' + init.strip("/"))
            targetData = readWithException(targetFile)
            if try_wrap:
                init_base = os.path.basename(init)
                # init_dir = os.path.dirname(init)
                initwrap = os.path.join("/wrap_"+init_base)
                wrappedFile = os.path.join(targetDir, 'image', initwrap.strip("/"))

                os.rename(targetFile, wrappedFile)            
                with open(targetDir + '/wrapped_file', 'w') as wrapFile:
                    wrapFile.write(wrappedFile)
                wrapFile.close()

                out = open(targetFile, "w")
                out.write("#!/bin/sh\n\n")
                out.write("if [ ! -e /tmp/STARTED ]; then\n")
                out.write("/firmadyne/busybox touch /tmp/STARTED;\n")
                out.write('/firmadyne/busybox sync;\n')
                out.write("%s &\n" % initwrap)
                out.write("fi\n\n")
                out.close()

                os.chmod(targetFile, stat.S_IRWXU | stat.S_IRGRP | stat.S_IXGRP | stat.S_IROTH | stat.S_IXOTH)
            skip_wrap = False
    ...
    return qemuInitValue, networkList, targetFile, targetData, ports, skip_wrap
...
def process(iid, arch, endianness, makeQemuCmd=False, outfile=None):
    ...
    init_list = (open(SCRATCHDIR + "/" + str(iid) + "/init").read().split('\n')[:-1])
    print("[*] init script list", init_list)
    for init in init_list:
        for try_wrap in [False, True]:
            if success:
                break
            if skip_wrap:
                skip_wrap = False
            ...
            if try_wrap:
                print("Trying", "wrap_"+init)
            else:
                print("Trying", init)

            with open(SCRATCHDIR + "/" + str(iid) + "/current_init", 'w') as out:
                out.write(init)
            qemuInitValue, networkList, targetFile, targetData, ports, skip_wrap = inferNetwork(iid, arch, endianness, init, try_wrap)
            ...
            if filterNetworkList:
                ...
                # archive the qemu data used
                init_tag = init.strip("/").replace("/", ".")
                print("archiving "+"/qemu.initial.serial."+init_tag+".log")
                if os.path.exists(SCRATCHDIR + '/' + str(iid) + '/qemu.initial.serial.'+init_tag+'.log'):
                    os.remove(SCRATCHDIR + '/' + str(iid) + '/qemu.initial.serial.'+init_tag+'.log')
                shutil.copyfile(SCRATCHDIR + '/' + str(iid) + '/qemu.initial.serial.log',
                                SCRATCHDIR + '/' + str(iid) + '/qemu.initial.serial.'+init_tag+'.log')
                print("archiving "+"/qemu.final.serial."+init.strip("/")+".log")
                if os.path.exists(SCRATCHDIR + '/' + str(iid) + '/qemu.final.serial.'+init_tag+'.log'):
                    os.remove(SCRATCHDIR + '/' + str(iid) + '/qemu.final.serial.'+init_tag+'.log')
                shutil.copyfile(SCRATCHDIR + '/' + str(iid) + '/qemu.final.serial.log',
                                SCRATCHDIR + '/' + str(iid) + '/qemu.final.serial.'+init_tag+'.log')
