...
class Exploit(Exploit):
    ...
    single_module = OptString("", "Use autopwn with only a single module")
    ...
    def run(self):
        ...
        if self.check_exploits:
            ...
            if self.single_module == "":
                ...
                path = "routersploit.modules."+self.single_module.replace("/", ".")
                module = utils.import_exploit(path)
                modules.append(module)
    ...
    def exploits_target_function(self, running, data):
        while running.is_set():
            try:
                ...
                # use custom auth creds if exists
                if self.username and self.password:
                    if hasattr(exploit, "username") and hasattr(exploit, "password"):
                        exploit.username = self.username
                        exploit.password = self.password
        ...
        #            exploit.target_protocol = "custom"
                ...
                if response is True:
                    print_info("\033[92m[+]\033[0m", "AAAAAAAAAA -> {}:{} {} {} is vulnerable".format(
                    ...
                    print_info("\033[91m[-]\033[0m", "AAAAAAAAAA -> {}:{} {} {} is not vulnerable".format(
                               exploit.target, exploit.port, exploit.target_protocol, exploit))
                else:
                    print_info("\033[94m[*]\033[0m", "AAAAAAAAAA -> {}:{} {} {} Could not be verified".format(
                               exploit.target, exploit.port, exploit.target_protocol, exploit))
